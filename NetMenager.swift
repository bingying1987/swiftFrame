...
{
    /*
    let shareInstance = NetManager()
    private let Manager:SessionManager
    private init(){
        Manager = Alamofire.SessionManager.default
        Manager.session.configuration.timeoutIntervalForRequest = 10 //10秒超时
    }
 */
    ...
    static func request(url:URLConvertible,method:HTTPMethod,parameters:Parameters?,encoding:ParameterEncoding,header:HTTPHeaders,bHud:Bool,complete:@escaping completedBlock)
    {
        ...
        Alamofire.SessionManager.default.session.configuration.timeoutIntervalForRequest = 10 //10秒超时
        /*
        Manager.request(url, method: method, parameters: parameters, encoding: encoding, headers: header).responseJSON { (response) in
            if bHud
            {
